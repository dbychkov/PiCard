...
2. **Audio I?S output** — GPIO 44 (DATA), GPIO 45 (BCLK), GPIO 46 (LRCK)
3. **SD card SPI** — GPIO 40–43
4. **Joystick + UART** — GPIO 32–37 (shared I?C/UART/DIV\_OFF)
...
| 32 | SDA | `PIN_I2C_SDA` | I?C0 SDA ? PCF8575 (joystick) |
| 33 | SCL | `PIN_I2C_SCL` | I?C0 SCL ? PCF8575 |
...
| 44 | I2S\_DATA (I2S\_R) | `I2S_DATA_PIN` | PIO2 I?S serial data ? TDA1387T |
| 45 | I2S\_CLK (BCLK) | `I2S_CLK_BASE_PIN` | PIO2 I?S BCLK ? TDA1387T SCLK |
| 46 | I2S\_LRCK | `I2S_CLK_BASE_PIN+1` | PIO2 I?S LRCK ? TDA1387T LRCK |
...
> `pio2` — I?S audio output (`PIO_I2S`)  
> Because GPIOs 44–46 are in the upper bank (?32), the I?S PIO uses `pio_set_gpio_base(pio, 16)` to shift its GPIO window up by 16, a feature unique to RP2350.

---

### TDA1387T I?S DAC (U4)

The **TDA1387T** (Philips/NXP) is a complete 16-bit stereo I?S DAC in an SO-8 package. It accepts a standard Philips I?S serial stream (SCLK, LRCK, DATA) and produces two filtered analog voltages on A\_LEFT (pin 6) and A\_RIGHT (pin 8).

**I?S PIO state machine (`audio_i2s_program`):**

The 8-instruction PIO program in `src/i2s/i2s.c` implements the I?S protocol completely in hardware, using the **side-set** mechanism to toggle BCLK and LRCK simultaneously with each data bit:
...
**TDA1387T filter pin:**  
The FLT pin (pin 7) connects to an external RC network (C15 = 0.1 µF and associated passives) that programmes the internal 4th-order reconstruction filter cutoff frequency, suppressing I?S quantisation images above the audio band.

---

### PCF8575DBR I?C I/O Expander (U3)

The **PCF8575** provides 16 quasi-bidirectional I/O pins (P0–P7 and P10–P17) on a single I?C slave at address `0x20` (pins A0=A1=A2=GND). It is polled continuously by Core 0 at 400 kHz via I?C0 (GPIO 32/33).

**Joystick connectivity:**  
Port pins P0–P8 connect to J10 (JOY-1, the 9-pin DE-9 Atari/Kempston-standard connector) through 10 k? pull-up resistors (R20–R28). With no joystick connected all lines float high, giving an idle state of `0xFF`. When a direction or fire button is pressed, the corresponding P-pin is pulled to GND by the joystick's internal switch.
...
1. Reads the previous result directly from the I?C hardware RX FIFO.
2. Loads the new read command into the TX FIFO with `I2C_IC_DATA_CMD_CMD_BITS | STOP`.

This ping-pong approach finishes in a few CPU cycles and returns immediately, so Core 0's 9 µs audio tick is never stalled waiting for an I?C transaction.
...
**Status LED:**  
D6 (LED) with R46/R47 (10 k? each) is connected to the PCF8575 `~INT` pin, giving a visual indication of I?C activity.
...
TDA1387T A_RIGHT (pin 8)
    ? R39 / R40 (10 k?)       AC-coupling / input impedance set
    ? C12 (10 µF, electrolytic) DC blocking capacitor (HPF: fc ? 1.6 Hz)
    ? U5A (+) non-inverting input (pin 3)
    ? U5A output (pin 1)
    ? R41 (62 k?) feedback to U5A (?) input (pin 2)
         ? C16 (470 pF)        HF roll-off in feedback: fc = 1/(2?·62k·470p) ? 5.5 kHz
    ? R36 (620 ?)              Output series protection resistor (short-circuit / capacitive load)
    ? J4 (out_A, R channel)
    ? J5 (PJ325, Ring — right)
```
...
TDA1387T A_LEFT (pin 6)
    ...
    ? U5B (+) input (pin 5)
    ? U5B output (pin 7)
    ? R42 (62 k?) ? C17 (470 pF)  same HF filter
    ? R35 (620 ?)
    ? J3 (in_A, L channel)
    ? J5 (PJ325, Tip — left)
```
...
With R41 = R42 = 62 k? as the feedback resistor and the TDA1387T internal output impedance (~1 k? typical) as the effective `R_in`, the gain is approximately unity (?1 buffer) — the primary function is **impedance transformation** (TDA1387T high-Z output ? low-Z headphone drive) and **HF noise suppression** via the 470 pF cap in feedback.
...
1. **RP1 and RP2** are resistor-array components mounted inline between the edge connector and the RP2350B GPIO pins, providing individual series protection resistors (values labelled "10", meaning 10 k? in the schematic context) for every address and control line.

2. When a 5 V NemoBus signal drives a GPIO pin through 10 k?, the internal ESD clamp diode of the RP2350 (which conducts when V_pin > V_IOVDD + 0.5 V) sinks the excess current:
...
3. **R1–R8 and R20–R21** (also 10 k?) are individual series resistors on specific control signals (/M1, /WR, /RD, /IORQ, /MREQ) for the same purpose.
...
```
GPIO 35 feeds the base of a BC817 transistor (via a 10 k? base resistor R25). The transistor's collector, pulled to +5 V through R30 (300 ?), drives the NemoBus `/NMI` line. When GPIO 35 outputs LOW, the transistor conducts, pulling `/NMI` to near GND. When GPIO 35 is tri-stated (input), the transistor is off and the bus line floats to +5 V through R30 and the Spectrum's own pull-up.
      ...
      ? packs 32-bit word into i2s_data variable
         ? DMA transfers to PIO2 TX FIFO
            ? PIO2 state machine clocks bits ? GPIO44/45/46
               ? TDA1387T (U4) receives I?S stream, decodes to analog
                  ? A_LEFT / A_RIGHT pins (±500 mV approx)
                     ? C12/C11 (10 µF, DC block), R40/R39 (10 k? input)
                        ? MCP602 U5A (right), U5B (left) — buffer/filter stage
                           ? R36/R35 (620 ? output protection)
                              ? J5 PJ325 stereo 3.5 mm jack (Tip=L, Ring=R, Sleeve=GND)
```

The PJ325 is a standard 3.5 mm TRS jack (J5, labelled "Audio OUT"). R in the schematic adjacent to the jack represents the right channel, L represents the left. The output impedance presented to a headphone is approximately 620 ? (R36/R35) plus the op-amp's near-zero output impedance — suitable for driving 32 ? headphones directly.
...
The +5VA net (TDA1387T pin 5 / VA supply) is filtered separately by C8 (1 µF) and R36 forms a ?-filter with C8 to isolate digital noise from the analog supply.
...
Pull-up resistors R20–R28 (10 k? each) hold all PCF8575 inputs at logic HIGH when no joystick is connected. A joystick button shorts the corresponding pin to GND.
...
| 3 | GND | GND |
| 4 | +5 V | +5 V (via R14 1 k? current limit) |
| 5 | KB CLK1 | 38 (PIN\_PS2\_CLK) |

R12 and R13 (1 k? each) are **series protection resistors** in the CLK and DATA lines between the Mini-DIN connector and the GPIO pins. They limit ESD-discharge current and also limit the current when the 5 V PS/2 device drives a 3.3 V GPIO pin. Combined with the RP2350B's internal ESD clamp diodes (clamping at ?3.8 V), these keep the GPIO inputs safe:

$$
I_{clamp} = \frac{5\text{ V} - 3.8\text{ V}}{1\text{ k}\Omega} = 1.2\text{ mA}
$$

This is acceptable for brief ESD events; steady-state the PS/2 open-drain lines are pulled high through the keyboard's own ~10 k? pull-ups, so steady-state GPIO current is near zero.
...
Both buttons connect through 74HC04 inverter stages (which both debounce and provide 5V?3.3V CMOS threshold matching) before reaching the RP2350B GPIO or the bus transistor. R9 and R10 (10 k?) provide pull-ups on the switch inputs.
...
|-----|---------|-----|---------|
| D4 | R31 (300 ?) | +3.3 V\_2 | SD card / DivMMC active |
| D5 | R43 (300 ?) | +5 V rail | Board power present |
| D6 | R46/R47 (10 k?) | PCF8575 ~INT | Joystick activity / I?C interrupt |
