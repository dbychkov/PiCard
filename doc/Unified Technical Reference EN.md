...
|--------|------|-----------|---------------|
| A0–A15 | 8–23 | Input | Through RP1/RP2 resistor networks (10 k?) to ESD-clamp |
| D0–D7 | 24–31 | Bidirectional | Through 74HC245 U1 (A-side = 3.3 V, B-side = 5 V NemoBus) |
| /M1 | 3 | Input | RP2 ? GPIO 3 (series 10 k?) |
...
| /NMI | 35 | Open-drain | GPIO 35 ? BC817 Q3 ? R30 (300 ?) pull-up to +5 V ? NemoBus /NMI |
...
| I?S DAC | PIO2 | DATA=44, BCLK=45, LRCK=46 | TDA1387T U4 pins 3/1/2 |
| PCF8575 joystick expander | I?C0 (400 kHz) | SDA=32, SCL=33 | PCF8575DBR U3 pins 23/22 |
| PS/2 keyboard | PIO0 | CLK=38, DATA=39 | Mini-DIN-6 J6 pins 5/1 via R12/R13 (1 k?) |
...
src/i2s/
  i2s.c/h            PIO2 I?S audio output driver ? TDA1387T U4
...
src/i2c/
  i2c_joy.cpp/h      PCF8575 U3 I?C joystick driver (NES + Sega modes)
...
The GS card is a standalone Z80-based sound coprocessor board. The firmware emulates the entire card in software, using the on-module PSRAM for banked RAM storage and the TDA1387T I?S DAC for audio output.
...
GS Z80 (Core 0, software) ? channel1–channel4 (uint8_t, PCM 8-bit unsigned)
  ...
  ? TDA1387T U4 (I?S slave): DATA pin 3, SCLK pin 1, LRCK pin 2
...
### Turbo Sound — Dual AY-3-8910 on One I?S Stream
...
Identical final path to GS — both emulated devices share the single TDA1387T I?S DAC and the MCP602 analogue output stage. The TDA1387T receives one merged 32-bit I?S word per sample, containing the fully-mixed left and right channels from all simultaneously-active sound sources.
...
Whenever DivMMC is active, the `/NMI` line is pulled low via GPIO 35 ? BC817 Q3 ? R30 (300 ?) to +5 V ? NemoBus `/NMI`. This generates the hardware NMI that causes the ESXDOS ROM to intercept DOS calls. When DivMMC deactivates, GPIO 35 is switched back to input (high-impedance) and Q3 turns off, releasing the NMI line:
...
- R12 (1 k?) in series with CLK ? GPIO 38 (`PIN_PS2_CLK`)
- R13 (1 k?) in series with DATA ? GPIO 39 (`PIN_PS2_DATA`)

The 1 k? series resistors limit the ESD clamp current to ~1.2 mA when a 5 V PS/2 device drives a 3.3 V GPIO input, keeping the RP2350B safe while still providing valid logic levels (PS/2 open-drain lines pull-up through the keyboard's own 10 k? resistors, giving a steady-state GPIO input close to 3.3 V).
...
#### Kempston Joystick — PCF8575DBR (U3) via I?C0 ? J10 (DE-9)

The physical joystick plugs into **J10**, a 9-pin DE-9 connector that maps to the classic Atari/Kempston pin layout. Each directional and fire pin connects through a 10 k? pull-up resistor (R20–R28) to the corresponding P-port of **PCF8575DBR U3**:
...
PCF8575 is accessed via **I?C0** at 400 kHz (GPIO 32 = SDA, GPIO 33 = SCL, device address `0x20`, A0=A1=A2=GND). The firmware uses a custom non-blocking I?C function `i2c_transfer_non_blocking()` that:
1. Reads the previous I?C result from the hardware RX FIFO (result of the previous poll).
...
| Core | Function |
|------|----------|
| **Core 0** | Audio synthesis (TS, GS, mixer ? TDA1387T), GS Z80 emulation, PS/2 decode, I?C joystick polling, watchdog |
...
3. Every 3rd tick: PS/2 keyboard DMA buffer scan (`decode_PS2()`) and I?C joystick poll (`joy_proc()`).
...
### I?S Audio Path — DMA-Driven, CPU-Free

The `audio_i2s_program` (8 instructions, `pio2`) implements the I?S protocol entirely in PIO:
...
The net effect: the TDA1387T receives a continuous, jitter-free I?S stream entirely driven by DMA + PIO. Even if Core 0 is temporarily delayed by a long `ExecZ80()` call, the DMA pair will simply keep sending the last `i2s_data` value, producing a held sample (brief tonal artefact) rather than a hard pop.
...
| `JOY_I2C` | 1 | PCF8575 U3 I?C joystick expander |
...
### 1. I?C Joystick Latency via PCF8575 — Input Lag and NES Bit-Bang Stall

**Problem:**

The PCF8575 (U3) is polled via I?C0 at 400 kHz. A complete I?C read transaction (start + address byte + ACK + 2 data bytes + STOP) requires a minimum of **~45 µs** of bus time at 400 kHz. In the current `NES_MODE` path, `joy_proc()` performs a multi-step bit-bang sequence over the PCF8575 port — asserting LATCH, toggling CLK 8 times, reading data — with each step requiring a separate I?C write (to update port outputs) and a subsequent I?C read (to sample DATA). For a full NES shift cycle this is approximately **8 ? 2 ? 45 µs = 720 µs** minimum, spread over multiple calls to `joy_proc()`.
...
- **I?C FIFO overflow check**: `i2c_transfer_non_blocking()` checks `i2c_get_write_available(i2c) < 16` before issuing the next command. If the previous transaction has not yet completed (I?C bus still busy from a prior call), the function bails out and returns `false`, silently dropping that poll step. There is no retry or error indication to the caller.
...
- In NES mode: if I?C is congested, `joy_state` may freeze at the last fully-completed value for multiple frames.

**Recommended Fixes:**

1. **Use a dedicated GPIO for one joystick direction per physical output pin**, bypassing I?C entirely for the most latency-critical lines. The RP2350B has enough free GPIO capacity — Kempston needs only 5 bits.
2. **For NES mode**: Use the RP2350B's PIO to bit-bang the NES latch/clock protocol natively (identical to the `DENDY_JOY_3_PINS` path that already exists in the codebase, compiled in when `DENDY_JOY_3_PINS=1`). This would eliminate I?C latency entirely and reduce total poll time to a handful of PIO clock cycles.
3. **For PCF8575 parallel mode** (Sega/Atari): Switch from I?C0 to a polled I?C burst (read both bytes in a single transaction). Pre-compute the full I?C command sequence as a DMA transfer triggered by a repeating timer, writing results into a memory buffer that `get_joy_data()` reads lock-free.
...
### 3. I?S Audio "Held Sample" Artifact — Not a Traditional Underrun but a Functional Equivalent

**Problem:**

The I?S DMA system uses a **single static `uint32_t i2s_data`** variable as its read source (no ring buffer). The DMA is configured with `channel_config_set_read_increment(&cfg_dma, false)`, meaning it reads from the same address (`&i2s_data`) on every transfer. The PIO2 DREQ signal paces these transfers at exactly **44,100 Hz**.
...
3. **check\_div\_status() and SD jumper detection**: This function may call `reset_spectrum()`, which executes `busy_wait_ms(10?50 = 500 ms)`. During this time, Core 0 is blocked, and the I?S DMA simply repeats the last sample for 500 ms — producing an audible "stuck note".
...
1. **Add a series 33–47 ? resistor** on each SD card's MISO line before they join the common net. This limits the short-circuit current to ~100 mA even in the worst case and prevents GPIO damage. The resistors also improve signal integrity by damping reflections on the MISO net.
...
|---|-------|----------|-----------------|----------------|
| 1 | PCF8575 I?C joystick latency (NES mode) | Medium | Kempston joystick | Medium — add PIO bit-bang path |
...
| 3 | I?S held-sample artefact during Core 0 stall | Low | All audio output | Medium — add small ring buffer |
...
| 5 | Dual SD connector MISO bus contention | Medium | SD/DivMMC | Low HW — add 33 ? series resistors |
